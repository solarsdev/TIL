...
- 관리형 NFS(network file system)으로 대규모 EC2에서 동시에 이용 가능
- EFS는 복수 AZ의 인스턴스들에서 동시에 사용 가능

- 고가용성, 분산형, 유연성, 고비용(gp2의 3배정도), 사용한 만큼 지불하는 특징이 있음
  - EBS는 용량과 성능을 미리 정해두고 비용을 내지만, EFS는 사용한 용량만큼 과금됨

![images/ec2_instance_store/7.png](images/ec2_instance_store/7.png)
...
- 사용 사례
  ...
  - 데이터 공유
  - 워드프레스 사이트
- 내부적으로 NFSv4.1 프로토콜을 이용
- EFS는 보안 그룹으로 방화벽 관리가 가능
- Linux 기반의 AMI와 호환됨 (윈도우 지원 하지 않음)
- KMS로 암호화 가능
- POSIX 파일 시스템의 스탠다드 파일 API를 이용
- 파일 시스템은 자동으로 확장되며 사용한만큼 과금되기 때문에 용량에 대한 계획이 불필요

### EFS의 퍼포먼스와 용량 클래스

- EFS Scale
  - 1000단위의 NFS 클라이언트 동시 접속, 10GB+/s의 처리량
  - 페타바이트 단위로 자동으로 용량 확장 가능
- Performance Mode (EFS 생성시 설정 가능)
  - 일반 (기본)
    - 저지연의 일반적인 사용사례 (웹서버, CMS, .. 등등)
  - Max I/O
    - 지연속도가 조금 느려지지만 처리량과 병렬처리량이 확연히 좋아짐 (빅데이터, 미디어 처리 등)
- Throughput mode (처리량 모드)
  - 버스팅 (1TB = 50MiB/s + burst of up to 100MiB/s)
  - 프로비저닝 원하는 만큼 추가로 설정 가능 버스팅에 추가로 1GiB 설정 등
- Storage Tier
  - 라이프사이클 매니저를 통해 파일을 N일 이후에 이동시킴
  - 스탠다드
    - 일반적으로 자주 이용되는 파일들
  - Infrequent Access (IA)
    - 저장에는 비용이 감소
    - 파일을 읽어들일때 비용이 청구됨
    - 따라서, 자주 사용되지는 않지만 저장이 필요한 파일에 적용 가능 (자주 엑세스 되지 않는)
- 가용성과 내구성
  - Standard
    - 일반적인 구성으로 복수 AZ에 분산저장됨 for Prod
  - One Zone
    - 개발용도등으로 비용절감책으로 활용가능
