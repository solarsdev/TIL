...
- 계정 내 거버넌스, 컴플라이언스를 감사하기 위한 툴로 사용
- 클라우드트레일은 기본적으로 활성화된 상태이다
- 계정 내 이벤트 히스토리나 API 요청에 대한 감사가 가능
  - Console
  - SDK
  - CLI
  - AWS 서비스들
- 클라우드트레일의 로그는 클라우드워치 로그 혹은 S3에 직접 저장할 수 있다
- 트레일은 모든 리전(디폴트) 혹은 싱글리전에 적용될 수 있다
- AWS 리소스가 삭제되거나 했을때는 클라우드트레일을 먼저 조사해보자

![images/x-ray/7.png](images/x-ray/7.png)

## 클라우드트레일 이벤트

- 관리 이벤트
  - AWS 계정 내 리소스에 대한 어떠한 액션들에 대한 로그
  - 예
    - 시큐리티 설정 변경 (IAM AttachRolePolicy)
    - 라우팅 데이터에 대한 룰 설정 (Amazon EC2 CreateSubnet)
    - 로깅 설정 (AWS CloudTrail CreateTrail)
  - 기본적으로 관리 이벤트에 대한 로그를 남기도록 설저됨
  - 로그는 읽기 이벤트(리소스에 대한 변경을 일으키지 않는 것)와 쓰기 이벤트(리소스에 대한 변경을 일으키는 것)로 나누어 생각할 수 있다
- 데이터 이벤트
  - 기본적으로 데이터 이벤트는 로그로서 남기지 않는다 (매우 높은 볼륨처리량을 요구하기 때문)
  - S3의 오브젝트 레벨단위의 변경 (GetObejct, DeleteObject, PutObject 등): 이것 또한 읽기, 쓰기 이벤트로 분류 가능
  - AWS 람다함수의 호출 이력 등 (Invoke API)

## CloudTrail Ingihts

- 클라우드트레일 인사이트를 활성화하여 일반적이지 않은 액티비티의 감지할 수 있다
  - 리소스 프로비전의 이상
  - 서비스 리밋
  - IAM 액션의 급증
  - 일반적인 관리 이벤트와는 다른 무언가
- 클라우드트레일 인사이트는 일반적인 관리 이벤트를 분석하여 베이스라인을 설정하고
- **읽기 이벤트**의 특정한 변화가 감지될 경우
  - 클라우드 트레일 콘솔
  - Amazon S3 이벤트 송신
  - 이벤트 브릿지의 이벤트 호출 등을 수행한다
    ![images/x-ray/8.png](images/x-ray/8.png)

## CloudTrail Retention

- 이벤트는 90일간 존속된다
- 그 이상의 데이터를 보관하고 싶다면 S3에 저장한 뒤 아테나로 분석하면 됨

![images/x-ray/9.png](images/x-ray/9.png)

## CloudTrail vs CloudWatch vs X-Ray

- CloudTrail
  - 유저, 서비스, AWS 콘솔에서 발생하는 API 요청을 감사하기 위함
  - 승인되지 않은 요청이나 변경점을 감지해낼 때 유용함
- ClouodWatch
  - 모니터링을 위해 지표를 수집
  - 어플리케이션의 로그를 저장
  - 예상 범위 밖의 지표를 감지하고 알람을 받기 위함
- X-Ray
  - 족적 분석을 자동화, 시각화된 서비스 맵
  - 지연시간, 에러와 실패를 분석
  - 분산 시스템 환경에서 요청을 추적하기 위함
